...
class _ChatRoomScreenState extends State<ChatRoomScreen> {
  ...
  Future<void> _sendText() async {
    ...
    _controller.clear();
    try {
      await _repo.sendMessage(chatId: widget.chatId, text: text, messageType: 'text');
    } catch (e) {
  ...
  Future<void> _pickImage(ImageSource source) async {
    ...
    final image = await picker.pickImage(source: source, imageQuality: 30, maxWidth: 600);
  ...
  Future<void> _toggleRecording() async {
    if (_isRecording) {
      ...
      if (await _audioRecorder.hasPermission()) {
        ...
        await _audioRecorder.start(const RecordConfig(encoder: AudioEncoder.aacLc, bitRate: 32000), path: path);
  ...
  Widget _buildMessageContent(Map<String, dynamic> data, bool mine) {
    ...
    if (type == 'image' && base64Data != null) {
      ...
      return Row(
        ...
        children: [
          Icon(Icons.insert_drive_file, color: mine ? Colors.white : Colors.blue),
          ...
          if (widget.otherUserId.isNotEmpty)
            StreamBuilder<DocumentSnapshot<Map<String, dynamic>>?>(
              ...
              builder: (context, snapshot) {
                ...
                // Update state for UI to disable input
                              ...
                              children: [
                                if (!mine && widget.otherUserId.isEmpty)
                                  Padding(
                                    ...
                                    child: Text(
                                      (data['senderEmail'] as String? ?? '').split('@').first,
                                    ...
                                    Flexible(
                                      child: GestureDetector(
                                        ...
                                        child: Container(
                                          ...
                                          child: _buildMessageContent(data, mine),
                                        ),
                                      ),
                                    ),
                                  ],
            ...
            child: _isBlocked
                ? Container(
                    ...
                    child: Column(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        const Text('You blocked this user.', style: TextStyle(color: Colors.grey, fontWeight: FontWeight.bold)),
                        const SizedBox(height: 8),
                        TextButton(
                          onPressed: () => _repo.unblockUser(widget.otherUserId),
                ...
                : Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        IconButton(
                          icon: Icon(_isRecording ? Icons.stop_circle : Icons.mic, color: _isRecording ? Colors.red : Colors.blue),
...
class _AudioBubbleState extends State<_AudioBubble> {
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        IconButton(
          icon: Icon(_isPlaying ? Icons.pause : Icons.play_arrow, color: widget.isMine ? Colors.white : Colors.black),
          onPressed: () {
            if (_isPlaying) {
              _player.pause();
            } else {
        ...
        SizedBox(
          ...
          child: Slider(
            ...
            inactiveColor: widget.isMine ? Colors.white54 : Colors.grey.shade400,
          ),
        ),
      ],
    );
